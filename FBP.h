    ...
    for (int f = 0; f < reconstruction.size().height; f++)
    {
        for (int c = 0; c < reconstruction.size().width; c++)
        {
            ...
            for (int t = 0; t < filtered_sinogram.size().width; t++)
            {
                ...
                if ((rho >= 0) && (rho < num_of_projection))
                    reconstruction.at<double>(f, c) += filtered_sinogram.at<double>(rho, t);
            }
            if (reconstruction.at<double>(f, c) < 0)
                reconstruction.at<double>(f, c) = 0;
        }
    }
...
void projection_filtered_timing(Mat src, Mat &filtered_sinogram)
{
    int height = src.rows;
    int weight = src.cols;
